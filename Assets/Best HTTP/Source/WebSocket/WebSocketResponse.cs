...
namespace BestHTTP.WebSocket
{
	public sealed class WebSocketResponse : HTTPResponse, IProtocol
	{
		...
		WebSocketFrameReader CloseFrame;

		ConcurrentQueue<WebSocketFrame> unsentFrames = new ConcurrentQueue<WebSocketFrame>();
		volatile AutoResetEvent newFrameSignal = new AutoResetEvent(false);
		int sendThreadCreated = 0;
		int closedThreads = 0;
		...
		/// </summary>
		CircularBuffer<int> rtts = new CircularBuffer<int>(RTTBufferCapacity);
			...
			: base(request, stream, isStreamed, isFromCache)
		{
			...
			ConnectionKey = new HostConnectionKey(baseRequest.CurrentUri.Host, HostDefinition.GetKeyForRequest(baseRequest));
		...
		public void Send(byte[] data)
		{
			...
			WebSocketFrame frame = new WebSocketFrame(WebSocket, WebSocketFrameTypes.Binary, new BufferSegment(data, 0, data.Length));
			Send(frame);
		}
		...
		public void Send(byte[] data, ulong offset, ulong count)
		{
			...
			WebSocketFrame frame = new WebSocketFrame(WebSocket, WebSocketFrameTypes.Binary, new BufferSegment(data, (int)offset, (int)count), true, true);
			Send(frame);
		}

		public void Send(WebSocketFrameTypes type, BufferSegment data)
		{
			WebSocketFrame frame = new WebSocketFrame(WebSocket, type, data, true, true, false);
			Send(frame);
		}
		...
		public void Send(WebSocketFrame frame)
		{
			...
			unsentFrames.Enqueue(frame);

			if (Interlocked.CompareExchange(ref sendThreadCreated, 1, 0) == 0)
			{
				HTTPManager.Logger.Information("WebSocketResponse", "Send - Creating thread", Context);

				PlatformSupport.Threading.ThreadedRunner.RunLongLiving(SendThreadFunc);
			}

			Interlocked.Add(ref _bufferedAmount, frame.Data.Count);
		...
		public void Close(ushort code, string msg)
		{
			...
			HTTPManager.Logger.Verbose("WebSocketResponse", string.Format("Close({0}, \"{1}\")", code, msg), Context);

			WebSocketFrame frame;
			while (unsentFrames.TryDequeue(out frame))
			{
		...
		void SendThreadFunc()
		{
			...
			try
			{
				bool mask = !HTTPProtocolFactory.IsSecureProtocol(baseRequest.CurrentUri);
				using (WriteOnlyBufferedStream bufferedStream = new WriteOnlyBufferedStream(Stream, 16 * 1024))
				{
					while (!closed && !closeSent)
					{
						...
						if (PingFrequnecy != TimeSpan.Zero)
						{
							...
							if (waitingForPong && now - lastPing > WebSocket.CloseAfterNoMessage)
							{
								HTTPManager.Logger.Warning("WebSocketResponse",
									string.Format(
										...
										lastPing, PingFrequnecy, WebSocket.CloseAfterNoMessage, now), Context);
						...
						try
						{
							...
							while (unsentFrames.TryDequeue(out frame))
							{
								...
								Interlocked.Add(ref _bufferedAmount, -originalFrameDataLength);
							}
					...
					HTTPManager.Logger.Information("WebSocketResponse", string.Format("Ending Send thread. Closed: {0}, closeSent: {1}", closed, closeSent),
						Context);
				}
			}
			catch (Exception ex)
			...
			finally
			{
				...
				HTTPManager.Logger.Information("WebSocketResponse", "SendThread - Closed!", Context);
			...
			try
			{
				while (!closed)
				{
					try
					{
						...
						if (HTTPManager.Logger.Level == Loglevels.All)
						{
							HTTPManager.Logger.Information("WebSocketResponse", "Frame received: " + frame.ToString(), Context);
						}
						...
						switch (frame.Type)
						{
							...
							case WebSocketFrameTypes.Ping:
								if (!closeSent && !closed)
								{
									Send(new WebSocketFrame(WebSocket, WebSocketFrameTypes.Pong, frame.Data, true, true));
								}
							...
							case WebSocketFrameTypes.Pong:
								try
								{
									...
									TimeSpan diff = TimeSpan.FromTicks(lastMessage.Ticks - lastPing.Ticks);
							...
							case WebSocketFrameTypes.ConnectionClose:
								HTTPManager.Logger.Information("WebSocketResponse", "ConnectionClose packet received!", Context);

								CloseFrame = frame;
								if (!closeSent)
								{
									Send(new WebSocketFrame(WebSocket, WebSocketFrameTypes.ConnectionClose, BufferSegment.Empty));
								}
				...
				HTTPManager.Logger.Information("WebSocketResponse", "Ending Read thread! closed: " + closed, Context);
			}
			finally
			{
				HTTPManager.Logger.Information("WebSocketResponse", "ReceiveThread - Closed!", Context);
		...
		void IProtocol.HandleEvents()
		{
			...
			while (CompletedFrames.TryDequeue(out frame))
			{
				...
				try
				{
					switch (frame.Type)
					{
						case WebSocketFrameTypes.Continuation:
							if (HTTPManager.Logger.Level == Loglevels.All)
							{
								HTTPManager.Logger.Verbose("WebSocketResponse", "HandleEvents - OnIncompleteFrame: " + frame.ToString(), Context);
							}
						...
						case WebSocketFrameTypes.Text:
							...
							if (HTTPManager.Logger.Level == Loglevels.All)
							{
								HTTPManager.Logger.Verbose("WebSocketResponse", "HandleEvents - OnText: " + frame.DataAsText, Context);
							}
						...
						case WebSocketFrameTypes.Binary:
							...
							if (HTTPManager.Logger.Level == Loglevels.All)
							{
								HTTPManager.Logger.Verbose("WebSocketResponse", "HandleEvents - OnBinary: " + frame.ToString(), Context);
							}
				...
				{
					HTTPManager.Logger.Exception("WebSocketResponse", string.Format("HandleEvents({0})", frame.ToString()), ex, Context);
				}
				finally
				{
					frame.ReleaseData();
			...
			{
				HTTPManager.Logger.Verbose("WebSocketResponse", "HandleEvents - Calling OnClosed", Context);
				try
				{
					ushort statusCode = 0;
				...
				{
					HTTPManager.Logger.Exception("WebSocketResponse", "HandleEvents - OnClosed", ex, Context);
				}
			}
		}
		...
		bool SendPing()
		{
			HTTPManager.Logger.Information("WebSocketResponse", "Sending Ping frame, waiting for a pong...", Context);
			...
			try
			{
				WebSocketFrame pingFrame = new WebSocketFrame(WebSocket, WebSocketFrameTypes.Ping, BufferSegment.Empty);

				Send(pingFrame);
			}
			catch
			{
				HTTPManager.Logger.Information("WebSocketResponse", "Error while sending PING message! Closing WebSocket.", Context);
		...
		void CloseWithError(HTTPRequestStates state, string message)
		{
		...
		void TryToCleanup()
		{
			if (Interlocked.Increment(ref closedThreads) == 2)
			{
				ProtocolEventHelper.EnqueueProtocolEvent(new ProtocolEventInfo(this));
				...
				HTTPManager.Logger.Information("WebSocketResponse", "TryToCleanup - finished!", Context);
			}
		}
