		...
		Proxy IProxyDetector.GetProxy(HTTPRequest request)
		{
			...
			if (detectedProxy != null && detectedProxy.Address != null)
			{
				...
				if (proxyUri != null && !proxyUri.Equals(request.CurrentUri))
				{
					...
					{
						HTTPManager.Logger.Warning(nameof(FrameworkProxyDetector),
							$"{nameof(IProxyDetector.GetProxy)} - FindFor returned with unknown format. proxyUri: '{proxyUri}'", request.Context);
					}
				}
			}
