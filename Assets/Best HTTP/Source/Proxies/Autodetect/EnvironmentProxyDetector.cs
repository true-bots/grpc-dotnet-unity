...
namespace BestHTTP.Proxies.Autodetect
{
	...
	public sealed class EnvironmentProxyDetector : IProxyDetector
	{
		...
		Proxy IProxyDetector.GetProxy(HTTPRequest request)
		{
			...
			{
				HTTPManager.Logger.Exception(nameof(EnvironmentProxyDetector), $"GetProxy - proxyUrl: '{proxyUrl}', exceptionList: '{exceptionList}'", ex,
					request.Context);
			}
		...
		{
			return Environment.GetEnvironmentVariable(key) ?? Environment.GetEnvironmentVariable(key.ToLowerInvariant());
		}
	}
}
#endif