...
namespace BestHTTP.Authentication
{
	...
	public sealed class Digest
	{
		...
		/// </summary>
		string QualityOfProtections { get; set; }
		...
		public void ParseChallange(string header)
		{
			...
			// Then process
			foreach (HeaderValue qp in qpl.Values)
			{
				switch (qp.Key)
				{
					case "basic":
			...
			try
			{
				switch (Type)
				{
					...
					case AuthenticationTypes.Digest:
					{
						...
						string cnonce = new Random(request.GetHashCode()).Next(int.MinValue, int.MaxValue).ToString("X8");
						...
						switch (Algorithm.TrimAndLower())
						{
							...
							case "md5-sess":
								...
								{
									HA1Sess = string.Format("{0}:{1}:{2}:{3}:{4}", credentials.UserName, Realm, credentials.Password, Nonce, ncvalue)
										.CalculateMD5Hash();
								}
						...
						string qop = QualityOfProtections != null ? QualityOfProtections.TrimAndLower() : null;
						...
						else if (qop.Contains("auth-int"))
						{
							...
							string HA2 = string.Format("{0}:{1}:{2}", method, uri, new BufferSegment(entityBody, 0, entityBody.Length).CalculateMD5Hash())
								.CalculateMD5Hash();
