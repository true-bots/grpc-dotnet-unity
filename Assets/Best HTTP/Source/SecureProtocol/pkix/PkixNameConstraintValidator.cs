...
namespace BestHTTP.SecureProtocol.Org.BouncyCastle.Pkix
{
	...
	{
		static readonly DerObjectIdentifier SerialNumberOid = X509Name.SerialNumber;

		ISet<Asn1Sequence> excludedSubtreesDN = new HashSet<Asn1Sequence>();

		ISet<string> excludedSubtreesDns = new HashSet<string>();

		ISet<string> excludedSubtreesEmail = new HashSet<string>();

		ISet<string> excludedSubtreesUri = new HashSet<string>();

		ISet<byte[]> excludedSubtreesIP = new HashSet<byte[]>();

		ISet<OtherName> excludedSubtreesOtherName = new HashSet<OtherName>();

		ISet<Asn1Sequence> permittedSubtreesDN;

		ISet<string> permittedSubtreesDns;

		ISet<string> permittedSubtreesEmail;

		ISet<string> permittedSubtreesUri;

		ISet<byte[]> permittedSubtreesIP;

		ISet<OtherName> permittedSubtreesOtherName;

		public PkixNameConstraintValidator()
		{
		}

		static bool WithinDNSubtree(
			Asn1Sequence dns,
		...
		ISet<Asn1Sequence> IntersectDN(ISet<Asn1Sequence> permitted, ISet<GeneralSubtree> dns)
		{
			...
			foreach (GeneralSubtree subtree1 in dns)
			{
				...
				{
					foreach (Asn1Sequence dn2 in permitted)
					{
		...
		ISet<Asn1Sequence> UnionDN(ISet<Asn1Sequence> excluded, Asn1Sequence dn)
		{
			if (excluded.Count < 1)
			{
				if (dn == null)
			...
			HashSet<Asn1Sequence> union = new HashSet<Asn1Sequence>();

			foreach (Asn1Sequence subtree in excluded)
			{
		...
		ISet<OtherName> IntersectOtherName(ISet<OtherName> permitted, ISet<GeneralSubtree> otherNames)
		{
		...
		void IntersectOtherName(OtherName otherName1, OtherName otherName2, ISet<OtherName> intersect)
		{
		...
		ISet<OtherName> UnionOtherName(ISet<OtherName> permitted, OtherName otherName)
		{
			HashSet<OtherName> union = permitted != null ? new HashSet<OtherName>(permitted) : new HashSet<OtherName>();
			union.Add(otherName);
			return union;
		}

		ISet<string> IntersectEmail(ISet<string> permitted, ISet<GeneralSubtree> emails)
		{
		...
		ISet<string> UnionEmail(ISet<string> excluded, string email)
		{
			if (excluded.Count < 1)
			{
				if (email == null)
		 ...
		 *         <code>ip</code>.
		 */
		ISet<byte[]> IntersectIP(ISet<byte[]> permitted, ISet<GeneralSubtree> ips)
		{
		 ...
		 */
		ISet<byte[]> UnionIP(ISet<byte[]> excluded, byte[] ip)
		{
			if (excluded.Count < 1)
			{
				if (ip == null)
			...
			HashSet<byte[]> union = new HashSet<byte[]>();
		 ...
		 */
		ISet<byte[]> UnionIPRange(byte[] ipWithSubmask1, byte[] ipWithSubmask2)
		{
		 ...
		 */
		ISet<byte[]> IntersectIPRange(byte[] ipWithSubmask1, byte[] ipWithSubmask2)
		{
		 ...
		 */
		byte[] IpWithSubnetMask(byte[] ip, byte[] subnetMask)
		{
		...
		bool IsOtherNameConstrained(OtherName constraint, OtherName otherName)
		{
			return constraint.Equals(otherName);
		}

		bool IsOtherNameConstrained(ISet<OtherName> constraints, OtherName otherName)
		{
		...
		void CheckPermittedOtherName(ISet<OtherName> permitted, OtherName name)
		{
		...
		void CheckExcludedOtherName(ISet<OtherName> excluded, OtherName name)
		{
		...
		bool IsEmailConstrained(string constraint, string email)
		{
		...
		bool IsEmailConstrained(ISet<string> constraints, string email)
		{
		...
		void CheckPermittedEmail(ISet<string> permitted, string email)
		{
			if (permitted != null
		...
		void CheckExcludedEmail(ISet<string> excluded, string email)
		{
		...
		bool IsDnsConstrained(string constraint, string dns)
		{
			return WithinDomain(dns, constraint) || Platform.EqualsIgnoreCase(dns, constraint);
		}

		bool IsDnsConstrained(ISet<string> constraints, string dns)
		{
			foreach (string constraint in constraints)
			{
		...
		void CheckPermittedDns(ISet<string> permitted, string dns)
		{
			if (permitted != null
		...
		void CheckExcludedDns(ISet<string> excluded, string dns)
		{
		...
		bool IsDirectoryConstrained(ISet<Asn1Sequence> constraints, Asn1Sequence directory)
		{
		...
		void CheckPermittedDirectory(ISet<Asn1Sequence> permitted, Asn1Sequence directory)
		{
			if (permitted != null
		...
		void CheckExcludedDirectory(ISet<Asn1Sequence> excluded, Asn1Sequence directory)
		{
		...
		bool IsUriConstrained(string constraint, string uri)
		{
		...
		bool IsUriConstrained(ISet<string> constraints, string uri)
		{
			foreach (string constraint in constraints)
			{
		...
		void CheckPermittedUri(ISet<string> permitted, string uri)
		{
			if (permitted != null
		...
		void CheckExcludedUri(ISet<string> excluded, string uri)
		{
		 ...
		 *         otherwise.
		 */
		bool IsIPConstrained(byte[] constraint, byte[] ip)
		{
			int ipLength = ip.Length;
		...
		bool IsIPConstrained(ISet<byte[]> constraints, byte[] ip)
		{
		 ...
		 */
		void CheckPermittedIP(ISet<byte[]> permitted, byte[] ip)
		{
			if (permitted != null
		 ...
		 */
		void CheckExcludedIP(ISet<byte[]> excluded, byte[] ip)
		{
		...
		bool WithinDomain(string testDomain, string domain)
		{
		 ...
		 */
		void UnionEmail(string email1, string email2, ISet<string> union)
		{
		...
		void UnionUri(string email1, string email2, ISet<string> union)
		{
			// email1 is a particular address
			if (email1.IndexOf('@') != -1)
			{
		...
		ISet<string> IntersectDns(ISet<string> permitted, ISet<GeneralSubtree> dnss)
		{
			HashSet<string> intersect = new HashSet<string>();
		...
		ISet<string> UnionDns(ISet<string> excluded, string dns)
		{
			if (excluded.Count < 1)
			{
				if (dns == null)
		 ...
		 */
		void IntersectEmail(string email1, string email2, ISet<string> intersect)
		{
			// email1 is a particular address
			if (email1.IndexOf('@') != -1)
			{
		...
		ISet<string> IntersectUri(ISet<string> permitted, ISet<GeneralSubtree> uris)
		{
			HashSet<string> intersect = new HashSet<string>();
		...
		ISet<string> UnionUri(ISet<string> excluded, string uri)
		{
			if (excluded.Count < 1)
			{
				if (uri == null)
		...
		void IntersectUri(string email1, string email2, ISet<string> intersect)
		{
			// email1 is a particular address
			if (email1.IndexOf('@') != -1)
			{
		...
		{
			Dictionary<int, ISet<GeneralSubtree>> subtreesMap = new Dictionary<int, ISet<GeneralSubtree>>();

			// group in ISets in a map ordered by tag no.
			foreach (Asn1Encodable element in permitted)
			{
			...
			foreach (KeyValuePair<int, ISet<GeneralSubtree>> entry in subtreesMap)
			{
		...
		string ExtractNameAsString(GeneralName name)
		{
		 ...
		 */
		static byte[] Max(byte[] ip1, byte[] ip2)
		{
		 ...
		 */
		static byte[] Min(byte[] ip1, byte[] ip2)
		{
			for (int i = 0; i < ip1.Length; i++)
			{
				if ((ip1[i] & 0xFFFF) < (ip2[i] & 0xFFFF))
		 ...
		 */
		static int CompareTo(byte[] ip1, byte[] ip2)
		{
		 ...
		 */
		static byte[] Or(byte[] ip1, byte[] ip2)
		{
		...
		int HashCollection(IEnumerable<byte[]> c)
		{
			int hash = 0;
			if (c != null)
			{
		...
		int HashCollection(IEnumerable<object> c)
		{
			int hash = 0;
			if (c != null)
			{
		...
		bool AreEqualSets(ISet<byte[]> set1, ISet<byte[]> set2)
		{
			if (set1 == set2)
			{
				return true;
		...
		bool AreEqualSets<T>(ISet<T> set1, ISet<T> set2)
		{
			if (set1 == set2)
			{
				return true;
		...
		string StringifyIPCollection(ISet<byte[]> ips)
		{
			string temp = "";
			temp += "[";
		...
		string StringifyOtherNameCollection(ISet<OtherName> otherNames)
		{
		...
		static void Append(StringBuilder sb, string name, object value)
		{
			sb.Append(name);
			sb.AppendLine(":");
			sb.Append(value);
