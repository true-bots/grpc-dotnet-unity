...
namespace BestHTTP.SecureProtocol.Org.BouncyCastle.Math.EC.Rfc8032
{
	...
	public static class Ed448
	{
		...
		const ulong M26UL = 0x03FFFFFFUL;
		const ulong M28UL = 0x0FFFFFFFUL;

		const int CoordUints = 14;
		const int PointBytes = CoordUints * 4 + 1;
		const int ScalarUints = 14;
		const int ScalarBytes = ScalarUints * 4 + 1;
		...
		// "SigEd448"
		static readonly byte[] Dom4Prefix = new byte[] { 0x53, 0x69, 0x67, 0x45, 0x64, 0x34, 0x34, 0x38 };

		static readonly uint[] P =
		{
		...
		{
			0xAB5844F3U, 0x2378C292U, 0x8DC58F55U, 0x216CC272U, 0xAED63690U, 0xC44EDB49U, 0x7CCA23E9U,
			0xFFFFFFFFU, 0xFFFFFFFFU, 0xFFFFFFFFU, 0xFFFFFFFFU, 0xFFFFFFFFU, 0xFFFFFFFFU, 0x3FFFFFFFU
		};

		const int L_0 = 0x04A7BB0D; // L_0:26/24
		const int L_1 = 0x0873D6D5; // L_1:27/23
		const int L_2 = 0x0A70AADC; // L_2:27/26
		const int L_3 = 0x03D8D723; // L_3:26/--
		const int L_4 = 0x096FDE93; // L_4:27/25
		const int L_5 = 0x0B65129C; // L_5:27/26
		const int L_6 = 0x063BB124; // L_6:27/--
		const int L_7 = 0x08335DC1; // L_7:27/22

		const int L4_0 = 0x029EEC34; // L4_0:25/24
		const int L4_1 = 0x01CF5B55; // L4_1:25/--
		const int L4_2 = 0x09C2AB72; // L4_2:27/25
		const int L4_3 = 0x0F635C8E; // L4_3:28/--
		const int L4_4 = 0x05BF7A4C; // L4_4:26/25
		const int L4_5 = 0x0D944A72; // L4_5:28/--
		const int L4_6 = 0x08EEC492; // L4_6:27/24
		const int L4_7 = 0x20CD7705; // L4_7:29/24

		static readonly uint[] B_x =
		{
			0x070CC05EU, 0x026A82BCU, 0x00938E26U, 0x080E18B0U, 0x0511433BU, 0x0F72AB66U, 0x0412AE1AU,
			0x0A3D3A46U, 0x0A6DE324U, 0x00F1767EU, 0x04657047U, 0x036DA9E1U, 0x05A622BFU, 0x0ED221D1U, 0x066BED0DU, 0x04F1970CU
		};

		static readonly uint[] B_y =
		{
			0x0230FA14U, 0x008795BFU, 0x07C8AD98U, 0x0132C4EDU, 0x09C4FDBDU, 0x01CE67C3U, 0x073AD3FFU,
			0x005A0C2DU, 0x07789C1EU, 0x0A398408U, 0x0A73736CU, 0x0C7624BEU, 0x003756C9U, 0x02488762U, 0x016EB6BCU, 0x0693F467U
		};

		const int C_d = -39081;

		const int WnafWidth = 5;
		const int WnafWidthBase = 7;

		// ScalarMultBase supports varying blocks, teeth, spacing so long as their product is in range [449, 479]
		const int PrecompBlocks = 5;
		const int PrecompTeeth = 5;
		const int PrecompSpacing = 18;
		const int PrecompRange = PrecompBlocks * PrecompTeeth * PrecompSpacing; // 448 < range < 480
		const int PrecompPoints = 1 << (PrecompTeeth - 1);
		const int PrecompMask = PrecompPoints - 1;

		static readonly object PrecompLock = new object();
		static PointAffine[] PrecompBaseWnaf = null;
		static uint[] PrecompBaseComb = null;
		...
		static byte[] CalculateS(byte[] r, byte[] k, byte[] s)
		{
			uint[] t = new uint[ScalarUints * 2];
			DecodeScalar(r, 0, t);
			uint[] u = new uint[ScalarUints];
			DecodeScalar(k, 0, u);
			uint[] v = new uint[ScalarUints];
			DecodeScalar(s, 0, v);
		...
		static bool CheckContextVar(byte[] ctx)
		{
			return ctx != null && ctx.Length < 256;
		}

		static int CheckPoint(uint[] x, uint[] y)
		{
			uint[] t = F.Create();
			uint[] u = F.Create();
			uint[] v = F.Create();
		...
		static int CheckPoint(uint[] x, uint[] y, uint[] z)
		{
			uint[] t = F.Create();
			uint[] u = F.Create();
			uint[] v = F.Create();
		...
		static bool CheckPointVar(byte[] p)
		{
		...
		static bool CheckScalarVar(byte[] s, uint[] n)
		{
		...
		static byte[] Copy(byte[] buf, int off, int len)
		{
		...
		static IXof CreateXof()
		{
			return new ShakeDigest(256);
		}

		static uint Decode16(byte[] bs, int off)
		{
			uint n = bs[off];
			n |= (uint)bs[++off] << 8;
			return n;
		...
		static uint Decode24(byte[] bs, int off)
		{
			uint n = bs[off];
			n |= (uint)bs[++off] << 8;
			n |= (uint)bs[++off] << 16;
		...
		static void Decode32(byte[] bs, int bsOff, uint[] n, int nOff, int nLen)
		{
		...
		static bool DecodePointVar(byte[] p, int pOff, bool negate, ref PointProjective r)
		{
		...
		static void Dom4(IXof d, byte phflag, byte[] ctx)
		{
		...
		static void Encode24(uint n, byte[] bs, int off)
		{
			bs[off] = (byte)n;
			bs[++off] = (byte)(n >> 8);
			bs[++off] = (byte)(n >> 16);
		...
		static void Encode56(ulong n, byte[] bs, int off)
		{
			Encode32((uint)n, bs, off);
			Encode24((uint)(n >> 32), bs, off + 4);
		}

		static int EncodePoint(ref PointProjective p, byte[] r, int rOff)
		{
			uint[] x = F.Create();
			uint[] y = F.Create();
...
#else
		static uint GetWindow4(uint[] x, int n)
#endif
		{
		...
		static sbyte[] GetWnafVar(uint[] n, int width)
		{
		...
		static void ImplSign(IXof d, byte[] h, byte[] s, byte[] pk, int pkOff, byte[] ctx, byte phflag,
		...
		static void ImplSign(byte[] sk, int skOff, byte[] ctx, byte phflag, byte[] m, int mOff, int mLen,
			byte[] sig, int sigOff)
		{
		...
		static void ImplSign(byte[] sk, int skOff, byte[] pk, int pkOff, byte[] ctx, byte phflag,
			byte[] m, int mOff, int mLen, byte[] sig, int sigOff)
		{
			if (!CheckContextVar(ctx))
			{
		...
		static bool ImplVerify(byte[] sig, int sigOff, byte[] pk, int pkOff, byte[] ctx, byte phflag,
		...
		static void Init(out PointAffine r)
		{
			r.x = F.Create();
			r.y = F.Create();
		}

		static void Init(out PointProjective r)
		{
			r.x = F.Create();
			r.y = F.Create();
			r.z = F.Create();
		...
		static void InvertZs(PointProjective[] points)
		{
		...
		static bool IsNeutralElementVar(uint[] x, uint[] y, uint[] z)
		{
			return F.IsZeroVar(x) && F.AreEqualVar(y, z);
		}

		static void PointAdd(ref PointAffine p, ref PointProjective r)
		{
			uint[] b = F.Create();
			uint[] c = F.Create();
			uint[] d = F.Create();
		...
		static void PointAdd(ref PointProjective p, ref PointProjective r)
		{
			uint[] a = F.Create();
			uint[] b = F.Create();
			uint[] c = F.Create();
		...
		static void PointAddVar(bool negate, ref PointAffine p, ref PointProjective r)
		{
			uint[] b = F.Create();
			uint[] c = F.Create();
			uint[] d = F.Create();
		...
		static void PointAddVar(bool negate, ref PointProjective p, ref PointProjective r)
		{
			uint[] a = F.Create();
			uint[] b = F.Create();
			uint[] c = F.Create();
		...
		static void PointCopy(ref PointProjective p, ref PointProjective r)
		{
			F.Copy(p.x, 0, r.x, 0);
			F.Copy(p.y, 0, r.y, 0);
			F.Copy(p.z, 0, r.z, 0);
		...
		static void PointDouble(ref PointProjective r)
		{
			uint[] b = F.Create();
			uint[] c = F.Create();
			uint[] d = F.Create();
		...
		static void PointLookup(int block, int index, ref PointAffine p)
		{
			...
			for (int i = 0; i < PrecompPoints; ++i)
			{
				...
				F.CMov(cond, PrecompBaseComb, off, p.x, 0);
				off += F.Size;
				F.CMov(cond, PrecompBaseComb, off, p.y, 0);
				off += F.Size;
			}
		}
...
#else
		static void PointLookup(uint[] x, int n, uint[] table, ref PointProjective r)
		{
			// TODO This method is currently hardcoded to 4-bit windows and 8 precomputed points
		...
		static void PointLookup15(uint[] table, ref PointProjective r)
		{
			int off = F.Size * 3 * 7;
		...
		static uint[] PointPrecompute(ref PointProjective p, int count)
		{
		...
		static void PointPrecomputeVar(ref PointProjective p, PointProjective[] points, int count)
		{
			Debug.Assert(count > 0);
		...
		static void PointSetNeutral(ref PointProjective p)
		{
			F.Zero(p.x);
			F.One(p.y);
			F.One(p.z);
		...
		public static void Precompute()
		{
			lock (PrecompLock)
			{
				...
				for (int i = wnafPoints; i < totalPoints; ++i)
				{
					...
					F.Copy(q.x, 0, PrecompBaseComb, off);
					off += F.Size;
					F.Copy(q.y, 0, PrecompBaseComb, off);
					off += F.Size;
				}
		...
		static void PruneScalar(byte[] n, int nOff, byte[] r)
		{
		...
		static byte[] ReduceScalar(byte[] n)
		{
			...
			x21 += x20 >> 28;
			x20 &= M28UL; // x21:58/--, x20:28/--
			x22 += x21 >> 28;
			x21 &= M28UL; // x22:57/54, x21:28/--
			x23 += x22 >> 28;
			x22 &= M28UL; // x23:45/42, x22:28/--
			x24 += x23 >> 28;
			x23 &= M28UL; // x24:28/18, x23:28/--
			...
			x18 += x17 >> 28;
			x17 &= M28UL; // x18:59/31, x17:28/--
			x19 += x18 >> 28;
			x18 &= M28UL; // x19:58/54, x18:28/--
			x20 += x19 >> 28;
			x19 &= M28UL; // x20:30/29, x19:28/--
			x21 += x20 >> 28;
			x20 &= M28UL; // x21:28/03, x20:28/--
			...
			x15 += x14 >> 28;
			x14 &= M28UL; // x15:59/31, x14:28/--
			x16 += x15 >> 28;
			x15 &= M28UL; // x16:59/32, x15:28/--
			x17 += x16 >> 28;
			x16 &= M28UL; // x17:31/29, x16:28/--
			x18 += x17 >> 28;
			x17 &= M28UL; // x18:28/04, x17:28/--
			...
			x01 += (ulong)((long)x00 >> 28);
			x00 &= M28UL;
			x02 += (ulong)((long)x01 >> 28);
			x01 &= M28UL;
			x03 += (ulong)((long)x02 >> 28);
			x02 &= M28UL;
			x04 += (ulong)((long)x03 >> 28);
			x03 &= M28UL;
			x05 += (ulong)((long)x04 >> 28);
			x04 &= M28UL;
			x06 += (ulong)((long)x05 >> 28);
			x05 &= M28UL;
			x07 += (ulong)((long)x06 >> 28);
			x06 &= M28UL;
			x08 += (ulong)((long)x07 >> 28);
			x07 &= M28UL;
			x09 += (ulong)((long)x08 >> 28);
			x08 &= M28UL;
			x10 += (ulong)((long)x09 >> 28);
			x09 &= M28UL;
		...
		static void ScalarMult(byte[] k, ref PointProjective p, ref PointProjective r)
		{
#if NETCOREAPP2_1_OR_GREATER || NETSTANDARD2_1_OR_GREATER || _UNITY_2021_2_OR_NEWER_
			...
			// Recode the scalar into signed-digit form
			{
				...
				uint c2 = Nat.ShiftDownBit(ScalarUints, n, c1);
		...
		static void ScalarMultBase(byte[] k, ref PointProjective r)
		{
#if NETCOREAPP2_1_OR_GREATER || NETSTANDARD2_1_OR_GREATER || _UNITY_2021_2_OR_NEWER_
		...
		static void ScalarMultOrderVar(ref PointProjective p, ref PointProjective r)
		{
		...
		static void ScalarMultStrausVar(uint[] nb, uint[] np, ref PointProjective p, ref PointProjective r)
		{
			Precompute();
