...
namespace BestHTTP.SecureProtocol.Org.BouncyCastle.Math.EC.Custom.Sec
{
	class SecT113R1Curve
		: AbstractF2mCurve
	{
		const int SECT113R1_DEFAULT_COORDS = COORD_LAMBDA_PROJECTIVE;
		const int SECT113R1_FE_LONGS = 2;
		static readonly ECFieldElement[] SECT113R1_AFFINE_ZS = new ECFieldElement[] { new SecT113FieldElement(BigInteger.One) };
			...
			: base(113, 9, 0, 0)
		{
			...
			m_coord = SECT113R1_DEFAULT_COORDS;
		}
		...
		public override ECLookupTable CreateCacheSafeLookupTable(ECPoint[] points, int off, int len)
		{
			ulong[] table = new ulong[len * SECT113R1_FE_LONGS * 2];
			{
				...
				for (int i = 0; i < len; ++i)
				{
					...
					Nat128.Copy64(((SecT113FieldElement)p.RawXCoord).x, 0, table, pos);
					pos += SECT113R1_FE_LONGS;
					Nat128.Copy64(((SecT113FieldElement)p.RawYCoord).x, 0, table, pos);
					pos += SECT113R1_FE_LONGS;
				}
			}
		...
		class SecT113R1LookupTable
			: AbstractECLookupTable
		{
			readonly SecT113R1Curve m_outer;
