...
namespace BestHTTP.SecureProtocol.Org.BouncyCastle.Math.EC.Custom.Sec
{
	class SecT163K1Curve
		: AbstractF2mCurve
	{
		const int SECT163K1_DEFAULT_COORDS = COORD_LAMBDA_PROJECTIVE;
		const int SECT163K1_FE_LONGS = 3;
		static readonly ECFieldElement[] SECT163K1_AFFINE_ZS = new ECFieldElement[] { new SecT163FieldElement(BigInteger.One) };
			...
			: base(163, 3, 6, 7)
		{
			...
			m_coord = SECT163K1_DEFAULT_COORDS;
		}
		...
		public override ECLookupTable CreateCacheSafeLookupTable(ECPoint[] points, int off, int len)
		{
			ulong[] table = new ulong[len * SECT163K1_FE_LONGS * 2];
			{
				...
				for (int i = 0; i < len; ++i)
				{
					...
					Nat192.Copy64(((SecT163FieldElement)p.RawXCoord).x, 0, table, pos);
					pos += SECT163K1_FE_LONGS;
					Nat192.Copy64(((SecT163FieldElement)p.RawYCoord).x, 0, table, pos);
					pos += SECT163K1_FE_LONGS;
				}
			}
		...
		class SecT163K1LookupTable
			: AbstractECLookupTable
		{
			readonly SecT163K1Curve m_outer;
