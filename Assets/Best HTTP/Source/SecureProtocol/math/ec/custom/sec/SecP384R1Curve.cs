...
namespace BestHTTP.SecureProtocol.Org.BouncyCastle.Math.EC.Custom.Sec
{
		...
		: AbstractFpCurve
	{
		...
		const int SECP384R1_DEFAULT_COORDS = COORD_JACOBIAN;
		const int SECP384R1_FE_INTS = 12;
		static readonly ECFieldElement[] SECP384R1_AFFINE_ZS = new ECFieldElement[] { new SecP384R1FieldElement(BigInteger.One) };
		...
		public override ECLookupTable CreateCacheSafeLookupTable(ECPoint[] points, int off, int len)
		{
			uint[] table = new uint[len * SECP384R1_FE_INTS * 2];
			{
				...
				for (int i = 0; i < len; ++i)
				{
					...
					Nat.Copy(SECP384R1_FE_INTS, ((SecP384R1FieldElement)p.RawXCoord).x, 0, table, pos);
					pos += SECP384R1_FE_INTS;
					Nat.Copy(SECP384R1_FE_INTS, ((SecP384R1FieldElement)p.RawYCoord).x, 0, table, pos);
					pos += SECP384R1_FE_INTS;
				}
			}
		...
		{
			readonly SecP384R1Curve m_outer;
