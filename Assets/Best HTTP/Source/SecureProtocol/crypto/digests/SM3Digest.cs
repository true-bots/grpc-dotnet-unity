...
namespace BestHTTP.SecureProtocol.Org.BouncyCastle.Crypto.Digests
{
	...
	public class SM3Digest
		: GeneralDigest
	{
		const int DIGEST_LENGTH = 32; // bytes
		const int BLOCK_SIZE = 64 / 4; // of 32 bit ints (16 ints)

		uint[] V = new uint[DIGEST_LENGTH / 4]; // in 32 bit ints (8 ints)
		uint[] inwords = new uint[BLOCK_SIZE];
		int xOff;
		...
		static readonly uint[] T = new uint[64];
