...
namespace BestHTTP.Connections
{
	...
	public sealed class KeepAliveHeader
	{
		...
		public void Parse(List<string> headerValues)
		{
			...
			if (parser.TryGet("timeout", out value) && value.HasValue)
			{
				...
				{
					TimeOut = TimeSpan.FromSeconds(intValue - 1);
				}
				else
				{
	...
	{
		public static void HandleResponse(string context, HTTPRequest request, out bool resendRequest, out HTTPConnectionStates proposedConnectionState,
			ref KeepAliveHeader keepAlive, LoggingContext loggingContext1 = null, LoggingContext loggingContext2 = null, LoggingContext loggingContext3 = null)
		{
			resendRequest = false;
			...
			if (request.Response != null)
			{
				...
				switch (request.Response.StatusCode)
				{
					...
					case 401:
					{
						...
						{
							Digest digest = DigestStore.GetOrCreate(request.CurrentUri);
					...
					case 308: // http://tools.ietf.org/html/rfc7238
					{
						...
						if (!string.IsNullOrEmpty(location))
						{
							...
							if (HTTPManager.Logger.Level == Loglevels.All)
							{
								HTTPManager.Logger.Verbose("HTTPConnection",
									string.Format("[{0}] - Redirected to Location: '{1}' redirectUri: '{1}'", context, location, redirectUri), loggingContext1,
									loggingContext2, loggingContext3);
							}

							if (redirectUri == request.CurrentUri)
							{
								HTTPManager.Logger.Information("HTTPConnection", string.Format("[{0}] - Redirected to the same location!", context), loggingContext1,
									loggingContext2, loggingContext3);
								goto default;
							}
							...
							{
								HTTPManager.Logger.Information("HTTPConnection", string.Format("[{0}] OnBeforeRedirection returned False", context), loggingContext1,
									loggingContext2, loggingContext3);
								goto default;
							}
						...
						{
							throw new Exception(string.Format("[{0}] Got redirect status({1}) without 'location' header!", context,
					...
					case 304:
						...
						{
							HTTPManager.Logger.Verbose("HTTPConnection", string.Format("[{0}] - HandleResponse - Loaded from cache failed!", context), loggingContext1,
								loggingContext2, loggingContext3);
							resendRequest = true;
						}
				...
				if (request.Response != null && !request.Response.IsClosedManually)
				{
					...
					{
						List<string> keepAliveheaderValues = request.Response.GetHeaderValues("keep-alive");
...
#if !BESTHTTP_DISABLE_CACHING
		public static bool LoadFromCache(string context, HTTPRequest request, LoggingContext loggingContext1 = null, LoggingContext loggingContext2 = null,
			LoggingContext loggingContext3 = null)
		{
			if (request.IsRedirected)
			{
				...
				{
					HTTPCacheService.DeleteEntity(request.RedirectUri);
				}
			}
		...
		static bool LoadFromCache(string context, HTTPRequest request, Uri uri, LoggingContext loggingContext1 = null, LoggingContext loggingContext2 = null,
			LoggingContext loggingContext3 = null)
		{
			if (HTTPManager.Logger.Level == Loglevels.All)
			{
				HTTPManager.Logger.Verbose("HTTPConnection", string.Format("[{0}] - LoadFromCache for Uri: {1}", context, uri.ToString()), loggingContext1,
					loggingContext2, loggingContext3);
			}

			HTTPCacheFileInfo cacheEntity = HTTPCacheService.GetEntity(uri);
			if (cacheEntity == null)
			{
			...
			{
				int bodyLength;
				using (Stream cacheStream = cacheEntity.GetBodyStream(out bodyLength))
				{
					if (cacheStream == null)
					{
						return false;
		...
		public static bool TryLoadAllFromCache(string context, HTTPRequest request, LoggingContext loggingContext1 = null, LoggingContext loggingContext2 = null,
			LoggingContext loggingContext3 = null)
		{
			...
			{
				request.Response = null;
				HTTPManager.Logger.Verbose("ConnectionHelper", string.Format("[{0}] - TryLoadAllFromCache - failed to load content!", context), loggingContext1,
					loggingContext2, loggingContext3);
		...
		public static Uri GetRedirectUri(HTTPRequest request, string location)
		{
			...
			if (result == null)
			{
				string baseURL = request.CurrentUri.GetComponents(UriComponents.SchemeAndServer, UriFormat.Unescaped);

				if (!location.StartsWith("/"))
				{
					string[] segments = request.CurrentUri.Segments;
