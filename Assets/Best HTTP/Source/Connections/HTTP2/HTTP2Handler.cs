...
namespace BestHTTP.Connections.HTTP2
{
	public sealed class HTTP2Handler : IHTTPRequestHandler
	{
		...
		{
			get { return this.goAwaySentAt == DateTime.MaxValue && this.isRunning; }
		}

		// Connection preface starts with the string PRI * HTTP/2.0\r\n\r\nSM\r\n\r\n).
		private static readonly byte[] MAGIC = new byte[24]
		...
		{
			get { return this.goAwaySentAt == DateTime.MaxValue ? TimeSpan.MaxValue : TimeSpan.FromMilliseconds(Math.Max(this.Latency * 2.5, 1500)); }
		}
								...
								else
								{
									...
									if (HTTPManager.Logger.Level == Loglevels.All)
										HTTPManager.Logger.Warning("HTTP2Handler",
											string.Format("No stream found for id: {0}! Can't deliver frame: {1}", header.StreamId, header), this.Context,
											http2Stream.Context);
								}
							}
							else
										...
										PluginEventHelper.EnqueuePluginEvent(
											new PluginEventInfo(PluginEvents.HTTP2ConnectProtocol,
												new HTTP2ConnectProtocolInfo(this.conn.LastProcessedUri.Host,
													this.settings.MySettings[HTTP2Settings.ENABLE_CONNECT_PROTOCOL] == 1 &&
													this.settings.RemoteSettings[HTTP2Settings.ENABLE_CONNECT_PROTOCOL] == 1)));
										break;
							...
							switch (frame.Type)
							{
								case HTTP2FrameTypes.DATA:
									...
									if (frame.PayloadLength > this.remoteWindow)
									{
										...
										HTTPManager.Logger.Warning("HTTP2Handler",
											string.Format("Data sending halted for this round. Remote Window: {0:N0}, frame: {1}", this.remoteWindow, frame.ToString()),
											this.Context);
										continue;
									}
