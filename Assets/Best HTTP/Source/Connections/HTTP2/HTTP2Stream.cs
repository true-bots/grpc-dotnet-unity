	...
	public class HTTP2Stream
	{
		...
		public virtual void Assign(HTTPRequest request)
		{
			...
			HTTPManager.Logger.Information("HTTP2Stream",
				string.Format("[{0}] Request assigned to stream. Remote Window: {1:N0}. Uri: {2}", this.Id, this.remoteWindow, request.CurrentUri.ToString()),
				this.Context, request.Context, this.parent.Context);
		...
		protected void ProcessIncomingFrames(List<HTTP2FrameHeaderAndPayload> outgoingFrames)
		{
			...
			while (this.incomingFrames.Count > 0)
			{
				...
				if ((this.isRSTFrameSent || this.AssignedRequest.IsCancellationRequested) && frame.Type != HTTP2FrameTypes.HEADERS &&
				...
				switch (frame.Type)
				{
						...
						if ((frame.Flags & (byte)HTTP2HeadersFlags.END_HEADERS) != 0)
						{
							...
							catch (Exception ex)
							{
								HTTPManager.Logger.Exception("HTTP2Stream",
									string.Format("[{0}] ProcessIncomingFrames - Header Frames: {1}, Encoder: {2}", this.Id, this.headerView.ToString(),
										this.encoder.ToString()), ex, this.Context, this.AssignedRequest.Context, this.parent.Context);
							}
						...
						if (HTTPManager.Logger.Level == Logger.Loglevels.All)
							HTTPManager.Logger.Information("HTTP2Stream",
								string.Format("[{0}] Received Window Update: {1:N0}, new remoteWindow: {2:N0}, initial remote window: {3:N0}, total data sent: {4:N0}",
									this.Id, windowUpdateFrame.WindowSizeIncrement, this.remoteWindow + windowUpdateFrame.WindowSizeIncrement,
									this.settings.RemoteSettings[HTTP2Settings.INITIAL_WINDOW_SIZE], this.sentData), this.Context, this.AssignedRequest.Context,
								this.parent.Context);
					...
					default:
						HTTPManager.Logger.Warning("HTTP2Stream",
							string.Format("[{0}] Unexpected frame ({1}, Payload: {2}) in state {3}!", this.Id, frame, frame.PayloadAsHex(), this.State), this.Context,
							this.AssignedRequest.Context, this.parent.Context);
						break;
				}
			...
			if (windowUpdate > 0 && this.State != HTTP2StreamStates.Closed)
			{
				if (HTTPManager.Logger.Level <= Logger.Loglevels.All)
					HTTPManager.Logger.Information("HTTP2Stream",
						string.Format("[{0}] Sending window update: {1:N0}, current window: {2:N0}, initial window size: {3:N0}", this.Id, windowUpdate, this.localWindow,
							this.settings.MySettings[HTTP2Settings.INITIAL_WINDOW_SIZE]), this.Context, this.AssignedRequest.Context, this.parent.Context);
		...
		protected virtual void ProcessIncomingDATAFrame(ref HTTP2FrameHeaderAndPayload frame, ref uint windowUpdate)
		{
			...
			if (this.localWindow < frame.PayloadLength)
			{
				HTTPManager.Logger.Error("HTTP2Stream",
					string.Format("[{0}] Frame's PayloadLength ({1:N0}) is larger then local window ({2:N0}). Frame: {3}", this.Id, frame.PayloadLength, this.localWindow,
						frame), this.Context, this.AssignedRequest.Context, this.parent.Context);
			}
			else
		...
		protected virtual void ProcessOpenState(List<HTTP2FrameHeaderAndPayload> outgoingFrames)
		{
			...
			{
				HTTPManager.Logger.Information("HTTP2Stream", string.Format("[{0}] Skipping data sending as remote Window is {1}!", this.Id, this.remoteWindow),
			...
			if (this.AssignedRequest.OnUploadProgress != null)
				RequestEventHelper.EnqueueRequestEvent(new RequestEventInfo(this.AssignedRequest, RequestEvents.UploadProgress, this.sentData,
					this.uploadStreamInfo.Length));
		}
				...
				case HTTP2Settings.INITIAL_WINDOW_SIZE:
					...
					HTTPManager.Logger.Information("HTTP2Stream",
						string.Format(
							...
							this.Id, oldValue, newValue, newValue - oldValue, this.remoteWindow, this.sentData), this.Context, this.AssignedRequest.Context,
						this.parent.Context);
					break;
			}
		}
